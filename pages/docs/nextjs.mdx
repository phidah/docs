...
export const meta = {
  ...
  lastEdited: '2020-08-05T02:33:17.000Z',
}
...
`next export` allows your app to be statically exported, however, it's not required for Vercel deployments as Next.js pages will be automatically exported to HTML thanks to the hybrid nature of pages. And Vercel will automatically serve the static assets from the [global edge network](https://vercel.com/docs/v2/edge-network/overview)

With Next.js you don't have to lock into static only or serverless only, you are free to decide on the go, in a per-page basis.
